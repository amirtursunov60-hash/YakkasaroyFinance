...
| `20260620240000_statistics_ranges.sql` | **ожидает применения** | Коридор состояний у `statistics` (фикс §B5 анализа): добавляет `min_val`, `max_val`, `stat_type`, `sign`, `source` (идемпотентно). Нужно для состояний ХМС и для импорта определений статистик из ManaJet. pgTAP — `supabase/tests/statistics_ranges_test.sql`. Сопутствует Edge Function **`manajet-import-refs`** (импорт справочников ManaJet — FPFund→`funds`, IncomeCategory→`income_types`, ExpenseCategory→`expense_types`, Stat→`statistics` — прямо в операционные таблицы по `outer_id`; балансы/Реестр не трогает) и ночной cron `mj_import_refs`. После применения — перегенерировать `database.types.ts` |
