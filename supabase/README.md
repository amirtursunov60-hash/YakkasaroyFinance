...
| `20260615185500_ledger_hardening_audit_reconcile.sql` | **ожидает применения** | доводка леджера: триггер `fp_register_no_update` (леджер неизменяем — UPDATE запрещён), функция `fp_reconcile_balances()` (сверка SUM(fp_register) = баланс фонда/счёта), аудит `fp_periods` и `directives` (переоткрытие/закрытие периода теперь пишутся в `audit_log`) |
