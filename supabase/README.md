...
| `20260618150000_fund_op_reverse.sql` | **применён** | откат операции фонда (`docs/funds-spec.md` §7): в `fp_register` — `reverses_id` (на откатываемую строку) + индекс; RPC `fp_reverse_fund_op(id)` добавляет компенсирующую запись (Реестр неизменяем, не удаляем), возвращая деньги в исходный фонд. Откатываются `fund_transfer`/`fund_income`/`fund_return` (займы — через «Долг»); запрет повторного отката; овердрафт/закрытый период держат триггеры |
