...
| `20260620240000_statistics_ranges.sql` | **применён** | Коридор состояний у `statistics` (фикс §B5 анализа): добавляет `min_val`, `max_val`, `stat_type`, `sign`, `source` (идемпотентно). Нужно для состояний ХМС и для импорта определений статистик из ManaJet. pgTAP — `supabase/tests/statistics_ranges_test.sql`. Сопутствует Edge Function **`manajet-import-refs`** (импорт справочников ManaJet — FPFund→`funds`, IncomeCategory→`income_types`, ExpenseCategory→`expense_types`, Stat→`statistics` — прямо в операционные таблицы по `outer_id`; балансы/Реестр не трогает) и ночной cron `mj_import_refs`. Применено через Supabase MCP; `database.types.ts` перегенерирован. Первый импорт: income 31, expense 44, statistics 31 (28 с коридором), funds 103/115 (12 фондов-прототипов с совпадающими code+name не связаны автоматически — решение по слиянию за заказчиком) |
