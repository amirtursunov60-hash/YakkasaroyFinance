...
| `20260620200000_crm_banquets.sql` | **ожидает применения** | CRM банкетов (перевод модуля `crm` с моков на БД): enum `crm_lead_stage` (new/show/offer/contract/won/lost); таблицы `crm_halls` (залы, сид справочника прототипа), `crm_clients` (база клиентов: tag/phone), `crm_leads` (воронка: зал, дата, гости, бюджет, этап, источник). RLS на родных функциях (`is_fin_admin`/`has_location_access`, сетевые записи при `location_id is null`); FK-индексы; `outer_id`/`is_archived` везде. Брони залов — производная от заявок (зал+дата+этап), отдельной таблицы нет. **Применить:** Dashboard → SQL Editor (или Supabase MCP, когда доступен), затем перегенерировать `database.types.ts` |
