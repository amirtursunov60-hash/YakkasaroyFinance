-- Восстановление fp_pay_request: предыдущая миграция вкладки «Фонды»
-- (20260618120000) пересоздала функцию по устаревшей версии и потеряла два
-- инварианта: блокировку строки заявки `for update` (защита от двойной оплаты,
-- 20260615184000) и оплату по одобренной сумме `coalesce(approved_amount,
-- planned_amount)` (20260616150000). Возвращаем их и сохраняем новую проверку
-- режима фонда (накопительный нельзя использовать для оплаты заявок).
...
begin
  ...
  -- блокировка строки заявки от двойной оплаты (ledger_concurrency_guards)
