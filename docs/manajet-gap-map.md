...
- **Статистики:** ✅ `stat_type` (§1) — закрыт: справочник `STAT_TYPES` + хелпер `statTypeLabel` (`utils/stats.ts`), селект типа в форме статистики, показ на карточке/в списке; `fetchStatistics`/`createStatistic` читают и пишут `stat_type` (без миграции — колонка уже была). _Известны 4 типа из 33 (1/8/11/12); остальные показываются как «Тип N» до уточнения справочника у ManaJet._ Осталось: периодичность день/нед/мес (§5), дневные значения (§6), комментарий к значению (§8) — нужны миграции (нет колонок: `statistic_values` хранит только недельное `value` без даты/заметки); дерево постов `parent_id` (§10).
