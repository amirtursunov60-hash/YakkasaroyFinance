...
- **Задачи/CRM:** ✅ богатые статусы (§4) — Kanban-воронка (#167); ✅ задача↔клиент (§5) — `crm_leads.client_id`; ✅ дедлайны (§10, частично) — `tasks.due_date`/`crm_leads.due_date` есть (напоминаний/событий ещё нет); ✅ дашборд собственника (§14) — вью «Сводка собственника»; ✅ описание задачи (§3) — закрыт: `tasks.description` + поле в форме и показ на карточке; ✅ комментарии к задачам (§6) — закрыт: таблица `task_comments` + тред-обсуждение под карточкой задачи. Осталось: напоминания-события (§10), таймлайн по лиду (§16).
