...
> ℹ️ **Статус (01.07.2026):** техническая интеграция с ManaJet (зеркало `mj_*`, синк) демонтирована в PR #218. ManaJet остаётся **функциональным ориентиром** (доменная модель, состав функций) — документ живёт как роадмап доработок.
...
| 12 | План счетов как справочник | `SheetOfAccounts` | ✅ (таблица `chart_accounts`, секция «План счетов» во вкладке «Фонды») | low | L |
...
- ✅ **Рассылки клиентам (Massmail) + in-app уведомления** (Задачи §12,13) — **закрыты.** In-app уведомления: таблица `notifications` + триггеры + лента `NotifyBell` (+ напоминания по срокам, RPC `fp_generate_due_reminders`). Massmail: таблицы `massmail_campaigns`/`massmail_recipients`, раздел «Рассылки клиентам» в CRM — сегмент (база клиентов/лиды воронки, фильтры метка/тип события/источник/точка) → снимок получателей с телефонами → WhatsApp-ссылки (wa.me, макрос `{name}`) + копируемый список + отметка «отправлено». SMS/email-шлюз не подключён (по решению — отправка через WhatsApp/копию).
- ✅ **Per-period остатки счетов/фондов** (Фонды §2,9) — **закрыт**: RPC `fp_period_balances(period_id)` (read-only, вычисляет накопленный итог из Реестра до конца недели N — БЕЗ материализации снапшота, Реестр остаётся единственным источником истины) + строка «На конец недели N» на карточке фонда (вкладка Фонды, привязка к выбранной неделе). Сверено: для последнего периода = live-баланс (diff 0).
- ✅ **Бухгалтерский леджер двойной записи**: план счетов, ОСВ, проводки (Реестр §3,9,12,13) — **закрыт** (НДС/склад осознанно → iiko). Фундамент: ОСВ (RPC `fp_turnover_sheet`, вкладка «ОСВ · оборотка») + «План счетов» (`chart_accounts`). Проводки (AccountingReg §13): детерминированная проекция Реестра — таблица правил `posting_rules` (op_type × компонента cash/fund → Дт/Кт; сид 30 правил + 9 служебных счетов), RPC `fp_postings` (журнал за период, Дт/Кт + субсчёт фонд/счёт ДС) и `fp_chart_turnover` (ОСВ по плану счетов). Параллельного учёта нет — проводка вычисляется из `fp_register`, Σ Дт = Σ Кт по построению; сверено на проде (сальдо 50/84 = live-балансам, diff 0). UI: «Проводки · журнал» в Отчётах (+CSV), «По плану счетов» в ОСВ, «Правила проводок» во вкладке Фонды. pgTAP — `accounting_postings_test.sql`.

### Хвосты (med/low, вне уровней — точечные доработки)
> _Сверено с историей мержей (PR #145–#184) и колонками БД 26.06.2026 — статусы ниже отражают фактическое состояние, а не первоначальный план._
- **Доход:** ✅ фильтры по дате/клиенту/виду (§4–6) — закрыты (#171); ✅ документ-основание (§7) — закрыт: структурное поле `incomes.basis_document` (форма/карточка дохода); ✅ CRUD видов дохода (§8) — закрыт: секция «Виды дохода (D-коды)» во вкладке Доходы (фин-админам), дерево папка→листья, добавление листа/переименование/архив, RLS itypes_write = is_fin_admin().
- **Заявки:** маршрут согласования по шагам (§4); ✅ статус «отозвана» ≠ отклонена (§7) — закрыт (#175); ✅ возврат на доработку (§8) — закрыт (статус `revision` + кнопка «На доработку» в Директиве + переподача автором); адресация «кому» (§10).
- **Счета:** ✅ история платежей в карточке счёта (§4) — есть (лента оплат + отмена); ✅ поиск (§6) — закрыт (#171); ✅ удаление вложения (§9) — есть (`deleteAttachment` + AttachmentsBlock). ✅ печатная форма счёта (§8) — закрыт: кнопка «Печать счёта» в карточке счёта клиента → чистый HTML-документ счёта в новом окне (печать/«Сохранить как PDF»), реквизиты покупателя из контрагента (#186) + продавца из `src/data/company.js` (заполнить реальные ИНН/банк). Билдер `utils/invoicePrint.ts` (чистый, с экранированием, unit-тесты). Осталось: пагинация (§7), просрочка дебиторки клиентов (§12).
- **Фонды:** ✅ CRUD валют (§3) и ✅ курсов обмена (§4) — закрыты: write-RLS на `currencies` (insert/update = is_fin_admin) и `exchange_rates` (update/delete), RPC `fp_set_base_currency` (атомарно ровно одна базовая); секции «Валюты» и «Курсы валют» во вкладке Фонды (фин-админам, образец PaymentTypesManager). Осталось из валют: обмен между счетами/фондами (§5), мультивалютная агрегация (§6). ✅ _CRUD способов оплаты (§8) — закрыт: сворачиваемая секция «Способы оплаты» во вкладке Фонды (фин-админам), RLS ptypes_write = is_fin_admin()._
...
- **Статистики:** ✅ `stat_type` (§1) — закрыт: справочник `STAT_TYPES` + хелпер `statTypeLabel` (`utils/stats.ts`), селект типа в форме статистики, показ на карточке/в списке; `fetchStatistics`/`createStatistic` читают и пишут `stat_type` (без миграции — колонка уже была). _Известны 4 типа из 33 (1/8/11/12); остальные показываются как «Тип N» до уточнения справочника у ManaJet._ ✅ комментарий к значению (§8) — закрыт: колонка `statistic_values.description` (заметка к факту) + поле в форме ввода и показ; попутно добавлена UPDATE-политика RLS (раньше её не было — правка уже введённого значения молча не сохранялась). ✅ периодичность день/нед/мес (§5) и ✅ дневные/датированные значения (§6) — закрыты: колонка `statistics.frequency` (week — дефолт, обратная совместимость) + таблица `statistic_dated_values` (значение на дату; для месяца — 1-е число) с RLS-зеркалом `statistic_values`; в форме статистики — выбор периодичности, для day/month — ввод по дате/месяцу и график из датированных значений (недельная логика не тронута). ✅ дерево постов `parent_id` (§10) — закрыт: колонка `org_positions.parent_id` уже была, задействована в UI (выбор «руководителя» в форме поста, вложенный показ подчинённых в оргсхеме) + триггер защиты от циклов.
- **Контрагенты:** ✅ адреса (§19), ✅ ответственные лица (§20), ✅ физ/юрлицо+реквизиты (§21) — закрыты (#186: `counterparties.entity_type`/`address`/`bank_name`/`bank_account`/`bank_mfo`/`contact_person` + поля в форме и на карточке); ✅ вложения-договоры (§22) — закрыт: таблица `counterparty_attachments` (зеркало invoice/bill, тот же бакет `attachments`, путь `counterparties/`) + `AttachmentsBlock` на карточке контрагента. _Архив в UI (§23) — уже реализован вместе с #164 (тумблер «С архивом» + кнопка «В архив/Из архива» на карточке)._ **Все пункты по контрагентам закрыты.**
- **Задачи/CRM:** ✅ богатые статусы (§4) — Kanban-воронка (#167); ✅ задача↔клиент (§5) — `crm_leads.client_id`; ✅ дедлайны (§10, частично) — `tasks.due_date`/`crm_leads.due_date` есть (напоминаний/событий ещё нет); ✅ дашборд собственника (§14) — вью «Сводка собственника»; ✅ описание задачи (§3) — закрыт: `tasks.description` + поле в форме и показ на карточке; ✅ комментарии к задачам (§6) — закрыт: таблица `task_comments` + тред-обсуждение под карточкой задачи; ✅ таймлайн по лиду (§16) — закрыт: триггер аудита на `crm_leads` + точечная политика чтения `audit_log` (история видна только по своим лидам, прочий журнал закрыт) + блок «История» в карточке лида с человекочитаемой расшифровкой изменений. ✅ напоминания-события по задачам/лидам (§10) — закрыт: RPC `fp_generate_due_reminders` создаёт уведомления (колокольчик #185) по просроченным/сегодняшним задачам (мне назначенным) и лидам (где я ответственный, активная стадия), идемпотентно; вызывается при входе. Осталось из крупного: обмен валют/мультивалютность (Фонды §5/6), Massmail, бухледжер.
