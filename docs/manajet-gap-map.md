...
- **Счета:** ✅ история платежей в карточке счёта (§4) — есть (лента оплат + отмена); ✅ поиск (§6) — закрыт (#171); ✅ удаление вложения (§9) — есть (`deleteAttachment` + AttachmentsBlock). Осталось: пагинация (§7), печатная форма PDF (§8), просрочка дебиторки клиентов (§12).
