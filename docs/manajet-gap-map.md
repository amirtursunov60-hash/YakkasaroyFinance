...
- **Статистики:** `stat_type` (§1) — колонка `statistics.stat_type` уже есть, осталось задействовать в UI (без миграции); периодичность день/нед/мес (§5), дневные значения (§6), комментарий к значению (§8) — нужны миграции (нет колонок: `statistic_values` хранит только недельное `value` без даты/заметки); дерево постов `parent_id` (§10).
