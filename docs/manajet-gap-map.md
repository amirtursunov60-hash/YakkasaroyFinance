...
- 🟠 **Бухгалтерский леджер двойной записи**: план счетов, ОСВ, проводки (Реестр §3,9,12,13) — крупнейший блок; часть (склад/НДС) уходит в iiko. **Фундамент заложен:** ОСВ (RPC `fp_turnover_sheet` поверх `fp_register`, вкладка «ОСВ · оборотка» в Отчётах — вход/приход/расход/исход по фондам и счетам ДС за период) + справочник «План счетов» (`chart_accounts`, секция во вкладке «Фонды»). Осталось: сами проводки двойной записи (AccountingReg §13), привязка плана счетов к Реестру, НДС/склад (→ iiko).
