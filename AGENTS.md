...
При ревью Pull Request смотреть в первую очередь:

- Регрессии бизнес-логики ХМС и финансового планирования.
- Ошибки в работе с Supabase, RLS, миграциями и типами БД.
- Нарушение инвариантов `fp_register`, фондов, периодов, заявок и счетов.
- Случайное изменение терминов, справочников, ролей, прав и стартовых данных.
- Поломки мобильной адаптации, тёмной/светлой темы и токенов палитры.
- Попадание секретов, `.env`, ключей, паролей или production credentials.
- Отсутствие нужных проверок: `typecheck`, `test`, `lint`, `build`.

Формат ревью: сначала баги/риски с точными файлами и строками, затем короткие вопросы, затем краткое резюме. Не оставлять шумные замечания по стилю, если они не влияют на поведение, безопасность или поддержку.
