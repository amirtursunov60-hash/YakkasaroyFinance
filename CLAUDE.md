...
Схема — SQL-миграции в `supabase/migrations/` (001–004: профили/роли, ядро, финансовый контур, сиды из прототипа). Применяются вручную через Supabase Dashboard → SQL Editor, инструкция — `supabase/README.md`. Правила:

- Любое изменение схемы — **только новым файлом миграции** (005, 006, …); применённые файлы не редактировать.
- Балансы фондов и счетов ДС не хранятся — считаются по Реестру (`fp_register`); запись дохода в реестр делает триггер.
- Права — RLS-политики на основе `public.app_role()` (роль из `profiles`); новые таблицы всегда создавать с RLS.
- Миграции идемпотентны (`if not exists` / `on conflict do nothing`) — сохранять этот стиль.
