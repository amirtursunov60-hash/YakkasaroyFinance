...
import { RotateCcw, Ban, ArrowRightLeft, Clock, Lock, Loader2, AlertCircle, CheckCircle2, Plus, X, List } from "lucide-react";
...
const OP_LABELS = {
  ...
  request_payment: "Оплата заявки", fund_transfer: "Перемещение",
...
export function Funds() {
  ...
  const load = useCallback(async () => {
    setErr("");
    try {
      const [fs, rs, opData, refData] = await Promise.all([
  ...
  return (<>
    ...
    <div style={{ ...st.cardWrap, marginTop: 18 }}>
      <section style={st.card}>
        ...
        {funds.map((f) => (
          <div key={f.id} style={{ ...st.frow, gridTemplateColumns: GRID }} className="frow">
            <div style={st.fName}>
    ...
    {showAdd && refs && (
      <AddFundModal C={C} st={st} refs={refs}
...
// ---------------------------------------------------------------- Новый фонд
function AddFundModal({ C, st, refs, onClose, onSaved }) {
  const baseCur = refs.currencies.find((c) => c.is_base) || refs.currencies[0];
  const [f, setF] = useState({ code: "", name: "", kind: "working", isRestricted: false, locationId: "" });
  ...
  const submit = async () => {
    ...
    try {
      await createFund({
        ...
        isRestricted: f.isRestricted, locationId: f.locationId, currencyId: baseCur.id,
      });
      onSaved();
    } catch (e) {
