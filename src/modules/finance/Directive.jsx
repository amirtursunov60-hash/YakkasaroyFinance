...
function LevelCard({ sg, C, st, isMobile, pctOf, setPcts, busy, locked, folders, onCalc, onApprove, onReset, onResetApproved, onOpenCalc }) {
  ...
  // а до правого края экрана дотягивается только сама «Доступно»:
  // calc(100vw − 282px), где 282 = отступы слева до сетки (main 16 + рамка
  // карточки 1 + паддинг строки 8 = 25) + Название (166) + % (42) + калькулятор
  // (32) + правый край (16 + 1 = 17). Поэтому «Доступно» доходит ровно до края
  // на любом телефоне, а «Рассчитано» и «Одобрено» уходят за край под прокрутку.
  // minWidth:max-content держит ленту шире экрана (иначе прокрутки не будет).
  const GRID6 = isMobile
    ? "166px 42px 32px calc(100vw - 282px) 120px 120px"
