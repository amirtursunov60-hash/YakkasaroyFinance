...
function LevelCard({ sg, C, st, isMobile, pctOf, setPcts, busy, locked, folders, onCalc, onApprove, onReset, onResetApproved, onOpenCalc }) {
  ...
  // а «Доступно» тянется к правому краю, НО с отступом ~24px, чтобы число не
  // упиралось в край: calc(100vw − 306px), где 306 = отступы слева до сетки
  // (main 16 + рамка карточки 1 + паддинг строки 8 = 25) + Название (166) + %
  // (42) + калькулятор (32) + правый край (16 + 1 = 17) + поле справа (24).
  // На освободившиеся 24px заходит пустой левый край колонки «Рассчитано»
  // (число в ней прижато вправо) — визуально это и есть отступ.
  // minWidth:max-content держит ленту шире экрана (иначе прокрутки не будет).
  const GRID6 = isMobile
    ? "166px 42px 32px calc(100vw - 306px) 120px 120px"
