...
function LevelCard({ sg, C, st, isMobile, pctOf, setPcts, busy, locked, folders, onCalc, onApprove, onReset, onResetApproved, onOpenCalc }) {
  ...
  // На телефоне число «Доступно» прижимаем влево, к калькулятору (убираем пустой
  // провал в середине). Крупная сумма растёт вправо и не налезает на калькулятор.
  const availCol = isMobile ? { textAlign: "left", paddingLeft: 12, paddingRight: 0 } : {};
  ...
  const FundRow = ({ x, child }) => {
    ...
    return (
      <div style={{ ...frow6, ...(child ? { paddingLeft: 14 } : {}) }} className="frow">
        ...
        <div style={{ ...st.fNum, fontWeight: 700, ...availCol }}>{fmt(avail)}</div>
      ...
      <section style={st.card}>
        ...
        {sg.rows.length === 0 ? <div style={st.empty}>Фонды этого этапа не настроены</div> : (<>
          <div style={{ ...frow6, ...st.frowHead, ...(isMobile ? { padding: "11px 8px" } : {}) }}>
            ...
            <div />
            <div style={{ ...st.fNum, ...availCol }}>Доступно</div>
            ...
            return (
              <div key={fid}>
                <div style={{ ...frow6, cursor: "pointer", background: C.panel2 }} className="frow"
                  ...
                  <div style={st.fPct} />
                  <div />
                  <div style={{ ...st.fNum, fontWeight: 700, ...availCol }}>{fmt(fsum.avail)}</div>
          ...
          <div style={{ ...frow6, ...st.frowTotal }}>
            ...
            <div style={st.fPct} />
            <div />
            <div style={{ ...st.fNum, fontWeight: 700, ...availCol }}>{fmt(totals.avail)}</div>
