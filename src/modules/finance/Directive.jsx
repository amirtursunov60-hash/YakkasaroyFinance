...
function LevelCard({ sg, C, st, isMobile, pctOf, setPcts, busy, locked, folders, onCalc, onApprove, onReset, onResetApproved, onOpenCalc }) {
  ...
  // а «Доступно» тянется к правому краю, НО с отступом ~12px, чтобы число не
  // упиралось в край: calc(100vw − 294px), где 294 = отступы слева до сетки
  // (main 16 + рамка карточки 1 + паддинг строки 8 = 25) + Название (166) + %
  // (42) + калькулятор (32) + правый край (16 + 1 = 17) + поле справа (12).
  // На освободившиеся 12px заходит пустой левый край колонки «Рассчитано»
  ...
  const GRID6 = isMobile
    ? "166px 42px 32px calc(100vw - 294px) 120px 120px"
