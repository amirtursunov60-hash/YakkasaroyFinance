...
// ---------------------------------------------------------------- SUPPLIERS
// Живые данные (ТЗ v2 §4.1.6): счета поставщиков с РАЗДЕЛЬНЫМИ периодами
// одобрения и оплаты. Поданные счета видны всегда; одобренные/оплаченные —
// в своих неделях. Повторяющиеся счета (аренда, коммуналка) дублируются
// кнопкой «Повторить». Оплата — fp_pay_bill (Реестр + статус атомарно).
...
export function Suppliers() {
  ...
  const loadBills = useCallback(async () => {
    try { setBills(await fetchBills(periodId)); }
  ...
  return (<>
    <section style={st.hero}>
      ...
      <div style={st.heroContent}>
        <div style={st.heroTop}>
          <div>
            <div style={st.heroLabel}>Счета поставщиков · входящие счета на оплату</div>
          ...
          {canSubmit && (
            <button style={st.btnGreen} className="btn" onClick={() => { setPrefill(null); setShowForm(true); }}>
              <Plus size={15} /> {isMobile ? "Счёт" : "Добавить счёт"}
            </button>
          )}
        </div>
    ...
    {!filtered.length && (
      <div style={{ ...st.locCard, ...st.empty }}>
        {bills.length ? "Нет счетов с этим статусом" : "Счетов на этой неделе нет — добавьте первый кнопкой выше"}
      </div>
    )}
    ...
    {showForm && refs && (
      <BillForm C={C} st={st} isMobile={isMobile} profile={profile}
...
function BillForm({ C, st, isMobile, profile, groups, refs, funds, counterparties, prefill, onCounterpartiesChanged, onClose, onSaved }) {
  ...
  const submit = async () => {
    ...
    if (!f.counterpartyId) return setErr("Выберите поставщика");
    ...
    try {
      await insertBill({
        ...
        status: "submitted", created_by: profile.id,
      });
      onSaved();
    } catch (e) {
  ...
  return (
    <div style={st.mdOverlay} onClick={onClose}>
      <div style={st.mdCard} onClick={(e) => e.stopPropagation()}>
        <div style={st.mdHead}>
          <div style={st.mdTitle}>Счёт поставщика</div>
        ...
        <div style={{ ...st.mdGrid, ...(isMobile ? { gridTemplateColumns: "1fr" } : {}) }}>
          ...
          <label style={{ ...st.mdCheck, ...st.mdFull }}>
            ...
            Повторяющийся счёт (аренда, коммуналка) — после оплаты можно продублировать
          </label>
        </div>
